...
def ml_loop(side: str):
    ...
    ball_served = False
    def move_to(player, pred,lastplat,x_balllast,cb,cbo) : #move platform to predicted position to catch ball 
        if player == '1P':
            if scene_info["platform_1P"][0]+20  > (pred-10) and scene_info["platform_1P"][0]+20 < (pred+10):
                ...
                if b<0 and cb==True and cbo==False:
                    return 1
                elif b>0 and cb==True and cbo==False:
                    return 2
                elif b<0 and cbo==True and cb==False:
                    return 2
                elif b>0 and cbo==True and cb==False:
                    return 1
                elif b<0 and cb==True and cbo==True:
                    return 1
                elif b>0 and cb==True and cbo==True:
                    return 2
    ...
    def check_blocker_opposite(player,lastblockerpos,pred):
        if(player=='1P'):
            result=False
            if(scene_info["ball_speed"][1] > 0):
                s = ( (scene_info["platform_1P"][1]-5)-(scene_info["blocker"][1]+20) ) // scene_info["ball_speed"][1]
                ballpred=pred+(-1*scene_info["ball_speed"][0]*s)
                ...
                if(blockerpred+33>=ballpred and blockerpred-3<=ballpred):
                    result=True
            return result        
    def check_blocker(player,lastblockerpos,pred):
        ...
        if(player=='1P'):
            if(scene_info["ball_speed"][1] > 0):
                ...
                s = ( (scene_info["ball"][1])-(scene_info["blocker"][1]+20) ) // scene_info["ball_speed"][1]
                ballpred=scene_info["ball"][0]+(scene_info["ball_speed"][0]*s)
                blockerpred=scene_info["blocker"][0]+s*(scene_info["blocker"][0]-lastblockerpos)
                bound = ballpred // 200 # Determine if it is beyond the boundary
                if (bound > 0): # pred > 200 # fix landing position
                    if (bound%2 == 0) : 
    ...
    def hit_check(lastblockerpos):
        ...
        if(scene_info["ball_speed"][1] > 0 and scene_info["ball"][1]<261):
            ...
            s2 = ((scene_info["blocker"][1]+25)-(scene_info["ball"][1]))// scene_info["ball_speed"][1]
            while(True):
        ...
        if scene_info["ball_speed"][1] > 0 : # 球正在向下 # ball goes down
            ...
            if(newpos[0]==-1):
                ...
                if(newpos[2]==-1):
                    ...
                    pred = newpos[0]+(-1*abs(scene_info["ball_speed"][0])*x) 
                    print(pred)
                elif(newpos[2]==1):
                    x = ( scene_info["platform_1P"][1]-newpos[1] ) // scene_info["ball_speed"][1] # 幾個frame以後會需要接  # x means how many frames before catch the ball
                    pred = newpos[0]+(abs(scene_info["ball_speed"][0])*x) 
            ...
            if(scene_info["ball"][1]>400 and scene_info["ball"][1]<=420):
                ...
                cbo=check_blocker_opposite(player='1P',lastblockerpos=x_lastblocker,pred=pred)
            return move_to(player = '1P',pred = pred,lastplat=x_lastplat,x_balllast=x_balllast,cb=cb[0],cbo=cbo)
        else : # 球正在向上 # ball goes up
            cb=check_blocker(player='1P',lastblockerpos=x_lastblocker,pred=scene_info["platform_1P"])
            if(cb[0]==True):
                if(cb[2]%2==1):
                    cb[2]=-1
                else:
                    cb[2]=1
                s = ( (scene_info["platform_1P"][1]-5)-(scene_info["blocker"][1]+20) ) // abs(scene_info["ball_speed"][1])
                newballpred=cb[1]+(cb[2]*abs(scene_info["ball_speed"][0])*s) 
                bound = newballpred // 200 # Determine if it is beyond the boundary
                if (bound > 0): # pred > 200 # fix landing position
                    if (bound%2 == 0) : 
                        newballpred = newballpred - bound*200                    
                    else :
                        newballpred = 200 - (newballpred - 200*bound)
                elif (bound < 0) : # pred < 0
                    if (bound%2 ==1) :
                        newballpred = abs(newballpred - (bound+1) *200)
                    else :
                        newballpred = newballpred + (abs(bound)*200)
                return move_to(player = '1P',pred = newballpred,lastplat=x_lastplat,x_balllast=x_balllast,cb=False,cbo=cbo)
            else:
                return move_to(player = '1P',pred = 100,lastplat=x_lastplat,x_balllast=x_balllast,cb=cb[0],cbo=cbo)
    ...
    def ml_loop_for_2P(x_lastplat,x_balllast,x_lastblocker):  # as same as 1P
        ...
        cbo=False
        if scene_info["ball_speed"][1] > 0 : 
            return move_to(player = '2P',pred = 100,lastplat=x_lastplat,x_balllast=x_balllast,cb=cb[0],cbo=cbo)
        else : 
            ...
            return move_to(player = '2P',pred = pred,lastplat=x_lastplat,x_balllast=x_balllast,cb=cb[0],cbo=cbo)
