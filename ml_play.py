...
def ml_loop(side: str):
    ...
    def move_to(player, pred,lastplat,x_balllast,cb) : #move platform to predicted position to catch ball 
        if player == '1P':
            ...
            if scene_info["platform_2P"][0]+20  > (pred-10) and scene_info["platform_2P"][0]+20 < (pred+10):
                if scene_info["platform_2P"][0]+20  > (pred-6) and scene_info["platform_2P"][0]+20<(pred+6) :
                    a=scene_info["platform_2P"][0]-lastplat
                    b=scene_info["ball"][0]-x_balllast
                    if b>0 and cb==False:
                        return 1
                    elif b<0 and cb==False:
    ...
    def check_blocker(lastblockerpos,pred):
    ...
    def ml_loop_for_1P(x_lastplat,x_balllast,x_lastblocker): 
        ...
        if scene_info["ball_speed"][1] > 0 : # 球正在向下 # ball goes down
            ...
            if(scene_info["ball"][1]>400 and scene_info["ball"][1]<=420):
                cb=check_blocker(x_lastblocker,pred)
    ...
    def ml_loop_for_2P(x_lastplat,x_balllast):  # as same as 1P
        cb=False
        if scene_info["ball_speed"][1] > 0 : 
    ...
    while True:
        ...
        if not ball_served:
            ...
            if side == "1P":
                ...
                command = ml_loop_for_2P(x_lastplat=x_lastplat,x_balllast=x_last_position)
